...
// ===============================
const wipTeamStyles = {
    'Customer Journey Stream': { color: '#3b82f6', bg: 'rgba(59, 130, 246, 0.22)', icon: 'workspaces' },
    'Fulfillment & Lifecycle Stream': { color: '#ea580c', bg: 'rgba(234, 88, 12, 0.22)', icon: 'local_shipping' },
    'Enterprise Systems Enablement Stream': { color: '#9333ea', bg: 'rgba(147, 51, 234, 0.22)', icon: 'domain' },
    'Foundational Services Stream': { color: '#0d9488', bg: 'rgba(13, 148, 136, 0.22)', icon: 'account_tree' },
    'Data Platform Team': { color: '#4f46e5', bg: 'rgba(79, 70, 229, 0.22)', icon: 'database' },
    'Technical Platform Team': { color: '#16a34a', bg: 'rgba(22, 163, 74, 0.22)', icon: 'settings' }
};
...
function getWipFilterOptions(rows) {
    const layers = [];
    const teams = [];
    const domains = [];
    ...
    rows.forEach(row => {
        ...
        if (!domains.includes(row.domainType)) domains.push(row.domainType);
    });

    return { layers, teams, domains };
}

function renderWipFilterGroup(containerId, options, filterKey, activeValue) {
    const container = document.getElementById(containerId);
    if (!container) return;
    const buttons = ['All', ...options].map(value => {
        const active = value === activeValue ? 'active' : '';
        return `<button class="wip-filter-btn ${active}" data-filter="${filterKey}" data-value="${value}">${value}</button>`;
    }).join('');
    container.innerHTML = buttons;
}

function bindWipFilters() {
    if (wipFiltersBound) return;
    document.querySelectorAll('.wip-filter-options').forEach(container => {
        container.addEventListener('click', event => {
            const button = event.target.closest('.wip-filter-btn');
            if (!button) return;
            const filterKey = button.dataset.filter;
            const value = button.dataset.value;
            wipFilterState[filterKey] = value;
            renderWipSection();
        });
    });
    wipFiltersBound = true;
}

function applyWipFilters(rows) {
...
function renderWipFilters() {
    const options = getWipFilterOptions(WIP_FLOW_ROWS || []);
    renderWipFilterGroup('wip-filter-layer', options.layers, 'layer', wipFilterState.layer);
    renderWipFilterGroup('wip-filter-team', options.teams, 'team', wipFilterState.team);
    renderWipFilterGroup('wip-filter-domain', options.domains, 'domain', wipFilterState.domain);
}
...
function renderWipFlowTable(rows) {
    ...
    const header = `
        <thead>
            <tr>
                ...
                <th scope="col">Accountable Team</th>
                <th scope="col">Bounded Context</th>
                <th scope="col">Flow (Expected Outcome)</th>
                <th scope="col">Start Event</th>
                <th scope="col">End Event</th>
                <th scope="col">Value Delivered</th>
                <th scope="col">Moments of Truth</th>
                <th scope="col">Features & Capabilities</th>
                <th scope="col">Domain Type</th>
            </tr>
        </thead>`;

    if (!rows.length) {
        table.innerHTML = `${header}<tbody><tr class="wip-empty"><td colspan="10">No flows match the current filters.</td></tr></tbody>`;
        return;
    }

    const layerOrder = [];
    (WIP_FLOW_ROWS || []).forEach(row => {
        if (!layerOrder.includes(row.layer)) layerOrder.push(row.layer);
    });
    ...
    layerOrder.forEach(layer => {
        ...
        body += `<tr class="wip-group-row"><td colspan="10"><span class="wip-group-title">${layer}</span><span class="wip-group-count">${layerRows.length} flows</span></td></tr>`;
        layerRows.forEach((row, idx) => {
            const style = wipTeamStyles[row.team] || { color: '#94a3b8', bg: 'rgba(148, 163, 184, 0.18)', icon: 'groups' };
            const rowClass = idx % 2 === 1 ? 'wip-row-alt' : '';
            const teamChip = `
            ...
            const featureList = row.features && row.features.length
                ? `<ul class="wip-feature-list">${row.features.map(f => `<li>${f}</li>`).join('')}</ul>`
                : '—';
...
function renderWipFlowCards(rows) {
    const container = document.getElementById('wip-flow-cards');
    if (!container) return;

    if (!rows.length) {
        container.innerHTML = '<div class="wip-empty">No flows match the current filters.</div>';
        return;
    }
    ...
    container.innerHTML = rows.map(row => {
        ...
        return `
            <article class="wip-card">
                ...
                <p class="wip-card-meta">${row.layer} · ${row.context}</p>
                <div class="wip-card-body">
                    <p><strong>Start:</strong> ${row.start}</p>
                    <p><strong>End:</strong> ${row.end}</p>
                    <p><strong>Value:</strong> ${row.value}</p>
                    <p><strong>Moments of truth:</strong> ${row.moments}</p>
                    <div><strong>Capabilities:</strong>${featureList}</div>
                </div>
            </article>`;
    }).join('');
}
...
function renderCompassTeams() {
    ...
    if (!container) return;
    container.innerHTML = (COMPASS_TEAMS || []).map(team => `
